...
const useMirrorMaterial = activeSection => {
  ...
  const mirrorTexture = useTexture({
    map: "/texture/MirrorColor.png",
  })
  ...
  useEffect(() => {
    ...
    if (mirrorTexture.map) {
      mirrorTexture.map.minFilter = THREE.LinearFilter
      mirrorTexture.map.magFilter = THREE.LinearFilter
      mirrorTexture.map.anisotropy = 1
    }
  }, [defaultEnvMap, mirrorTexture.map])

  return useMemo(() => {
    // Versão para a seção do espelho (aidatingcoach)
    if (activeSection === "aidatingcoach") {
      return new THREE.MeshStandardMaterial({
        map: mirrorTexture.map,
        opacity: 1,
        roughness: 0,
        metalness: 0,
    ...
    // Versão para outras seções (usando MeshPhysicalMaterial como solicitado)
    return new THREE.MeshPhysicalMaterial({
      ...
      roughness: 0,
      metalness: 0.3,
      envMap: defaultEnvMap, // Usando o envMap carregado
      envMapIntensity: 1.0,
    })
  }, [mirrorTexture.map, defaultEnvMap, activeSection])
}
//Hallos Material
